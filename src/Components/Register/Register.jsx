  ...
  const getAddress = async () => {
    try {
      ...
      if (!upline) {
        ...
        Swal.fire("Invalid upline ID. Please check and try again.");
        return;
      }

      const uplineaddress = await idToAddress(upline);
      if (!uplineaddress) {
        Swal.fire("Failed to get upline wallet address. Please try again.");
        return;
      }
      ...
      if (!chain || !chain.id) {
        ...
        Swal.fire("Switch to BSC Mainnet first.");
        return;
      }
      ...
      try {
        ...
        const approvetx = await USDTapprove("5000000000000000000");
        if (!approvetx) {
          Swal.fire("USDT Approval failed. Please try again.");
          return;
        }
        ...
        if (!receipt) {
          Swal.fire(
            "USDT Approval transaction failed. Please check your wallet."
          );
          return;
        }
        ...
        Swal.fire("An error occurred during USDT approval. Please try again.");
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (!registerReceipt) {
          ...
          Swal.fire(
            "Registration transaction failed. Please check your wallet."
          );
          return;
        }
        ...
        Swal.fire("An error occurred during registration. Please try again.");
        setLoading(false);
